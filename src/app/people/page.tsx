...
export default async function PeoplePage({
  ...
  const [member, { data }, params] = await Promise.all([
    ...
    createAdminClient()
      .from("people")
      .select(
        "id, full_name, email, headshot_url, headline, title, company, industry, location_label, latitude, longitude, grad_year, member_type, pledge_class, interests, bio, linkedin_url"
      )
      .order("full_name"),
    searchParams,
  ]);
